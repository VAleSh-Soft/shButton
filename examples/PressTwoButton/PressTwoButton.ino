 ...
 * 
 * @brief Пример обработки одновременного нажатия на две кнопки; под одновременным
 *        нажатием подразумевается ситуация, когда при возникновении события клика
 *        (или другого заданного в методе isSecondButtonPressed() события) одной
 *        кнопки, вторая кнопка находится в нажатом состоянии.
 * 
 *        Т.к. в этом скетче одновременное нажатие кнопок определяется по событию
 *        ONE_CLICK, то последовательность действий должна быть следующей:
 *        - нажать и удерживать нажатой первую кнопку;
 *        - сделать клик второй кнопкой;
 *        - после возникновения события двойного нажатия отпустить первую кнопку;
 * 
 *        При должной сноровке эти действия выполняются быстро и легко, и позволяют
 *        уверенно получать нужный результат
 * 
 * @version 2.1
 * @date 25.09.2026
 *
