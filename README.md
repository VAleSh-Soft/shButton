...
Процедура работает точно так же, но сама не считывает текущее положение контактов кнопки. Его нужно определять самостоятельно и передавать в метод в виде аргумента. Метод используется для работы с виртуальными кнопками, например, с аналоговой клавиатурой.
