...
*Важно: для надежной работы с кнопками метод `getButtonState()` нельзя вызывать более одного раза на один проход `loop()`. В противном случае вы рискуете пропускать события кнопки. Если результат вызова метода нужно использовать в нескольких местах, используйте.*
