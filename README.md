 ...
 Версия 1.4 - 
 * слегка оптимизирован код, объем оперативной памяти, занимаемой кнопкой, уменьшен c 30 до 20 байт;
...
который выдает true если кнопка нажата и false - если отпущена. Проверка выполняется по результату последнего опроса кнопки. Однако можно задать параметр true (btn.isButtonClosed(true)) - тогда перед проверкой положения кнопки будет вызван метод getButtonState(); однако для виртуальной кнопки параметр true будет в любом случае проигнорирован.
