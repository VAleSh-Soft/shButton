  shButton - библиотека для отработки нажатия кнопки. Возможности библиотеки:
  - Работа с кнопками с нормально разомкнутыми и нормально замкнутыми контактами;
  - Работа с подключением PULL_UP и PULL_DOWN;
  - Работа с виртуальной кнопкой, т.е. кнопкой, не привязанной к конкретному пину, например - кнопки аналоговой или матричной клавиатуры;
 ...
 Версия 1.4 - 
 * слегка оптимизирован код, уменьшен объем оперативной памяти, занимаемой кнопкой;
...
Второй вариант этого метода; метод используется для работы с виртуальными кнопками, например, с аналоговой клавиатурой

btn.getButtonState(isClosed);

Процедура работает точно так же, но сама не считывает текущее положение контактов кнопки. Его нужно определять самостоятельно и передавать в метод в виде аргумента: true - кнопка нажата, false - кнопка не нажата.
...
которая выдает true если кнопка нажата и false - если отпущена. Проверка выполняется по результату последнего опроса кнопки. Однако можно задать параметр true (btn.isButtonClosed(true)) - тогда перед проверкой положения кнопки будет вызван метод getButtonState(); однако для виртуальной кнопки параметр true будет в любом случае проигнорирован.
