...
где 80 - новое значение интервала антидребезга в милисекундах. Антидребезг можно отключить, если используется аппаратное подавление дребезга или, например, сенсорные кнопки, для этого достаточно указать нулевой интервал (btn.setDebounce(0)).
...
которая выдает true если кнопка нажата и false - если отпущена. Проверка выполняется по результату последнего опроса кнопки. Однако можно задать параметр true (btn.isButtonClosed(true)) - тогда перед проверкой положения кнопки будет выполнен вызван метод getButtonState().
