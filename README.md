...
### История версий
 ...
 + добавлена возможность установки произвольного пользовательского флага для кнопки; это может быть полезно, когда, например, функция, которой нужно считывать состояние кнопки, выполняется слишком редко и не может надежно получить состояние кнопки, опрашивая ее напрямую;
