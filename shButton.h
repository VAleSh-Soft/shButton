...
#define BTN_ONECLICK 5  // одиночный клик, следует через некоторый интервал после
                        // нажатия кнопки, если за это время не последовал двойной
                        // клик или длительное удержание кнопки нажатой;
                        // по умолчанию событие отключено
...
#define LCM_CONTINUED 0   // непрерывное событие BTN_LONGCLICK, генерируется
                          // постоянно, пока кнопка удерживается нажатой, если
                          // интервал удержания превышен; значение по умолчанию
#define LCM_ONLYONCE 1    // однократное событие BTN_LONGCLICK, генерируется
                          // только один раз по истечении интервала удержания
                          // кнопки, после чего будет постоянно выдаваться
                          // BTN_PRESSED до тех пор, пока кнопка будет удерживаться
                          // нажатой
#define LCM_CLICKSERIES 2 // по истечении интервала удержания кнопки событие
                          // BTN_LONGCLICK генерируется постоянно через равные
                          // интервалы времени, в промежутках выдается BTN_PRESSED

class shButton
{
private:
  uint8_t _btn_pin = NO_PIN;                         // пин, на который посажена кнопка
  ...
  uint16_t _interval_of_serial = INTERVAL_OF_SERIAL; // интервал следования события BTN_LONGCLICK, 
                                                     // если установлен режим LCM_CLICKSERIES, мс
   ...
   * 2 бит - тип кнопки - нормально разомкнутая (BTN_NO(0)) или нормально 
   *         замкнутая (BTN_NC(1))
   * 3 бит - флаг включения подавления дребезга - пока флаг поднят (1),
   *         изменения состояния не принимаются
  ...
  unsigned long btn_timer = 0; // таймер отработки подавления дребезга контактов 
                               // и длинного клика
  unsigned long dbl_timer = 0; // таймер двойного клика

  // получение мгновенного состояния кнопки - нажата/не нажата с учетом типа 
  // подключения и без учета дребезга контактов
...
public:
   ...
   * @param inputtype тип подключения (PULL_UP / PULL_DOWN), т.е. с подтяжкой 
   *                  к VCC или к GND;
   * @param btntype тип кнопки (BTN_NO / BTN_NC), т.е. с нормально разомкнутыми 
   *                или нормально замкнутыми контактами;
   */
  shButton(uint8_t pin, uint8_t inputtype = PULL_UP, uint8_t btntype = BTN_NO);

  /**
   * @brief конструктор виртуальной кнопки, без привязки к конкретному пину и 
   *        указания типа подключения и типа кнопки;
  ...
  /**
   * @brief получение состояния кнопки - отпущена/нажата/удерживается в режиме 
   *        виртуальной кнопки;
   *
  ...
  /**
   * @brief получение последнего состояния кнопки; возвращает статус кнопки по 
   *        результатам последнего вызова метода getButtonState();
   *
   * @return uint8_t, текущее состояние кнопки (BTN_RELEASED .. BTN_LONGCLICK);
   */
  ...
  /**
   * @brief возвращает true, если по результатам последнего вызова метода 
   *        getButtonState() кнопка нажата;
   *
  ...
  /**
   * @brief определение одновременного нажатия двух кнопок; возвращает true, 
   *        если в момент возникновения события btn_state текущей кнопки кнопка 
   *        _but уже нажата; если метод вернул true, состояние обеих кнопок 
   *        сбрасывается;
   *
   * @param _but вторая кнопка, состояние которой проверяется
   * @param btn_state событие, при возникновении которого проверяется состояние 
   *                  второй кнопки
  ...
  /**
   * @brief принудительный сброс состояния кнопки; может понадобиться, если по 
   *        каким-то причинам нужно, например, исключить возникновение событий 
   *        BTN_ONECLICK и BTN_DBLCLICK;
   *
   */
   ...
   *
   * @param inputtype тип подключения кнопки (PULL_UP - подтянута к VCC, 
   *                  PULL_DOWN - подтянута к GND);
   */
   ...
   *
   * @param btntype тип кнопки (BTN_NO - нормально разомкнутая, 
   *                BTN_NC - нормально замкнутая);
   */
  void setButtonType(uint8_t btntype);

  /**
   * @brief установка интервала подавления антидребезга (значение по умолчанию 
   *        50 мс); для отключения антидребезга нужно задать 0 мс;
   *
   * @param new_timeout новое значение интервала подавления антидребезга в 
   *                    милисекундах;
   */
   ...
   *
   * @param longclickmode режим обработки удержания кнопки (LCM_CONTINUED, 
   *                      LCM_ONLYONCE или LCM_CLICKSERIES);
   */
  void setLongClickMode(uint8_t longclickmode);

  /**
   * @brief установка интервала выдачи события BTN_LONGCLICK в режиме 
   *        LCM_CLICKSERIES (значение по умолчанию 200 мс); установка значения 
   *        0 переведет кнопку в режим LCM_ONLYONCE;
   *
...
uint8_t shButton::getButtonState(bool isClosed)
{
  ...
  // если поднят флаг подавления дребезга и интервал еще не вышел, больше 
  // ничего не делать
  ...
  { // и не поднят флаг подавления дребезга
    if (!getFlag(DEBOUNCE_BIT))
    {
      ...
      { // кнопка находится в отжатом состоянии
        ...
        { // если период двойного клика закончился, проверить на виртуальный клик
          //  и сбросить флаг одиночного клика
      ...
      { // кнопка находится в нажатом состоянии, но время удержания еще не вышло, 
        // и события удержания еще не было
        _btn_state = BTN_PRESSED;
      }
      else // если кнопка удерживается нажатой дольше времени удержания, то дальше 
           // возможны варианты
      {    // если события удержания еще не было, то выдать его
        ...
        else // если это уже не первое событие удержания, то дальше согласно 
             // выбранного режима
        {
  ...
  { // если задано подавление дребезга контактов
    ...
    { // если флаг подавления еще не поднят - поднять и больше ничего не делать
      if (!getFlag(DEBOUNCE_BIT))
      {
        ...
        // и заодно сбросить переменную _btn_state, чтобы не выскакивали 
        // множественные события типа BTN_DOWN пока не истечет интервал 
        // антидребезга; исключение - состояние удержания кнопки в режиме 
